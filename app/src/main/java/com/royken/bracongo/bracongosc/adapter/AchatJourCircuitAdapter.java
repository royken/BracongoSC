...
public class AchatJourCircuitAdapter extends BaseAdapter {

    private Context mContext;
    private LayoutInflater mInflater;
    private List<AchatMoisData> achatMoisData;

    public AchatJourCircuitAdapter(Context mContext, List<AchatMoisData> achatMoisData) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        LinearLayout layout;
        if (convertView == null) {
            layout = (LinearLayout) mInflater.inflate(R.layout.achat_jour_circuit_item, parent, false);
        } else {
        ...
        TextView tv_total = (TextView) layout.findViewById(R.id.totalMois);
        ...
        tv_total.setText((achatMoisData.get(position).getQteBi() + achatMoisData.get(position).getQteBg() + achatMoisData.get(position).getQtePet()) + "");

        tv_jour.setTag(position);
        tv_bi.setTag(position);
        tv_bg.setTag(position);
        tv_pet.setTag(position);
        tv_total.setTag(position);
        return layout;
    }
}
